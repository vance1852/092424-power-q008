...
class JsonApplication:
    ...
    def handle(self, method: str, target: str, headers: Mapping[str, str] | None = None, body: bytes = b"") -> Response:
        ...
        try:
            ...
            if method == "POST" and path == "/tariff/rules":
                return Response(201, self.service.create_tariff_rule(actor, payload))
            if method == "GET" and len(parts) == 2 and parts[0] == "tariff" and parts[1] == "resolve":
                return Response(200, self.service.resolve_tariff(query.get("series", [""])[0], query.get("date", [""])[0]))
            if len(parts) >= 2 and parts[:2] == ["tariff", "rules"]:
                if method == "GET" and len(parts) == 3:
                    return Response(200, self.service.tariff_rule(parts[2]))
                if method == "GET" and len(parts) == 4 and parts[3] == "preview":
                    return Response(200, self.service.preview_tariff(parts[2], query.get("date", [""])[0]))
                if method == "POST" and len(parts) == 4 and parts[3] == "review":
                    return Response(200, self.service.review_tariff_rule(actor, parts[2], int(payload["expected_revision"])))
                if method == "POST" and len(parts) == 4 and parts[3] == "publish":
                    return Response(200, self.service.publish_tariff_rule(actor, parts[2], int(payload["expected_revision"])))
                if method == "POST" and len(parts) == 4 and parts[3] == "retire":
                    return Response(200, self.service.retire_tariff_rule(actor, parts[2], int(payload["expected_revision"])))
            if method == "POST" and path == "/contracts":
                return Response(201, self.service.create_contract(actor, payload))
            if method == "GET" and len(parts) == 2 and parts[0] == "contracts":
                return Response(200, self.service.contract(parts[1]))
            if method == "POST" and path == "/settlements":
                return Response(201, self.service.settle_contract(actor, payload))
            if method == "GET" and len(parts) == 2 and parts[0] == "settlements":
                return Response(200, self.service.settlement_bill(parts[1]))
            if method == "GET" and len(parts) == 3 and parts[0] == "settlements" and parts[2] == "recalculations":
                return Response(200, self.service.recalculations(parts[1]))
            if method == "GET" and path == "/recalculations":
                return Response(200, self.service.recalculations())
