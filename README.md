...
- 峰平谷分时电价规则按系列版本管理，经历草稿、复核、生效、退役流程，正式发布内容不可覆盖，同日修订以更高版本生效；
- 售电合同签订时锁定当时生效的规则版本，旧合同继续按签订时的规则计费；
- 结算保存完整规则快照、输入电量与舍入过程，重复结算返回相同账单，规则更正只生成可追踪的重算建议；
- 运营人员可在规则发布前预览某个日期的分时应付电价（含节假日加价）；
...
- `src/power_dispatch/`：电价、设施、送出线路、燃料库存、提名、分时电价规则、合同结算、负荷情景、HTTP API 与离线验收；
...
该命令会在内存数据库中登记六个结算日的峰谷电价，创建电厂、终端和送出线路，完成燃料库存入账、提名分配、送电及负荷情景分析，随后走通分时电价规则的草稿、复核、生效流程，预览节假日应付电价，签订合同并按锁定规则结算，最后以同日修订触发重算建议，输出一行 JSON。成功时退出码为 `0` 且 `status` 为 `ok`。
...
健康检查为 `GET /health`。除健康检查外，请求通过 `X-Actor-Id` 携带操作者编号。可用接口覆盖电价、设施、送出线路、停运事件、燃料批次、提名、能力分配、送电、负荷情景、分时电价规则（`/tariff/rules`，含 `review`/`publish`/`retire`/`preview` 与 `/tariff/resolve`）、售电合同（`/contracts`）、结算账单（`/settlements`）与重算建议（`/recalculations`），以及审计链。服务重启后，SQLite 中的业务状态和历史版本会继续保留。
